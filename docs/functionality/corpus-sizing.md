...
- The line-count distribution is: **27** files measure 10,802 lines, **1** file (`filler.js`) measures 1,999 lines, and **1** file (`file_27.js`) measures 6,347 lines — 29 files in all, with no other line counts present (Source: Tech Spec §2.2.6; representative `society_mgmt_300k/src/controllers/file_0.js:L1` (standard), `society_mgmt_300k/src/middleware/file_27.js:L1` (short variant), `society_mgmt_300k/src/utils/filler.js:L1` (padding)).
- The canonical standard file is `society_mgmt_300k/src/controllers/file_0.js` at **10,802 lines** (Source: `society_mgmt_300k/src/controllers/file_0.js:L1` (line-1 header of the canonical standard file); Tech Spec §2.2.6 — 10,802 lines).
- `src/utils/filler.js` is **comment-only padding**: every line is of the form `// filler NNNNNN`, spanning the observed range `// filler 298001` … `// filler 299999`, for **1,999 lines** and **0 functions** (Source: `society_mgmt_300k/src/utils/filler.js:L1` … `society_mgmt_300k/src/utils/filler.js:L1999`).
- `src/middleware/file_27.js` is the **short variant** at **6,347 lines** (the only numbered file that is not 10,802 lines) (Source: `society_mgmt_300k/src/middleware/file_27.js:L1` (line-1 header of the short variant); Tech Spec §2.2.6 — 6,347 lines).
...
Equivalently, `27 × 10,802 = 291,654`, and `291,654 + 1,999 + 6,347 = 300,000` (Source: Tech Spec §2.2.6 — the 300,000-line corpus total).
...
- **Per function** — 8 lines of code (the `function` declaration, `let r=0;`, three `r+=x*n;` accumulations, the `if(r%2===0){r+=10}` branch, `return r;`, and the closing `}`) followed by **1 trailing blank line**, for **9 lines per function** (Source: `society_mgmt_300k/src/controllers/file_0.js:L3-L10` (the 8-line body) plus `society_mgmt_300k/src/controllers/file_0.js:L11` (the trailing blank line); Tech Spec §2.2.6).
...
- Standard files contain 1,200 functions and 1,200 trailing blank lines, giving `2 + 9 × 1,200 = 10,802` (Source: `society_mgmt_300k/src/controllers/file_0.js:L1`; Tech Spec §2.2.6 — 1,200 functions, 1,200 trailing blank lines).
- The short variant contains 705 functions and 705 trailing blank lines, giving `2 + 9 × 705 = 6,347` (Source: `society_mgmt_300k/src/middleware/file_27.js:L1`; Tech Spec §2.2.6 — 705 functions).

**`filler.js` is the documented exception to this formula.** It has **no header line, no `store` declaration, and no functions** — it is **1,999 lines** of comment-only padding (`// filler NNNNNN`) and therefore does not follow `2 + 9 × fn_count`. Its sole purpose is to pad the corpus to the exact 300,000-line total. This is a *verified absence* of code, not an omission (Source: `society_mgmt_300k/src/utils/filler.js:L1` … `society_mgmt_300k/src/utils/filler.js:L1999`; Tech Spec §2.2.6).
...
That is, `27 × 1,200 + 705 = 33,105` functions, with `filler.js` contributing **0** functions because it is comment-only padding (Source: Tech Spec §2.2.6 — corpus function total of 33,105; `society_mgmt_300k/src/utils/filler.js:L1` … `society_mgmt_300k/src/utils/filler.js:L1999` — comment-only padding, 0 functions). The per-layer distribution of these counts (which layer each numbered file belongs to) is tabulated in [`./module-reference.md`](./module-reference.md).
...
- `society_mgmt_300k/src/controllers/file_0.js:L1-L2` (line-1 header and line-2 `const store = [];`), `society_mgmt_300k/src/controllers/file_0.js:L3-L10` (the 8-line function body) — the canonical standard file: 10,802 lines, 1,200 functions.
- `society_mgmt_300k/src/middleware/file_27.js:L1` — the 6,347-line short variant with 705 functions.
- `society_mgmt_300k/src/utils/filler.js:L1` … `society_mgmt_300k/src/utils/filler.js:L1999` — 1,999 lines of comment-only padding (`// filler 298001` … `// filler 299999`), 0 functions.
- Tech Spec §2.2.6 — the deterministic 300,000-line composition (F-005) and the corpus function total of 33,105 across the 29 `.js` files.
